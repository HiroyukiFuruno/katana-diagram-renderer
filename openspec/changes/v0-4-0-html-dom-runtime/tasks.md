...
- [x] 4.3 KRR public API / dependency graph / rustfmt / clippy / workspace tests / `just coverage` を実行し、line coverage `100% / 0 uncovered` を通す。
  - Current verification: `rtk proxy env CARGO='rtk proxy cargo' just check` passed; AST lint, runtime asset checks, Chromium manifest/install helper checks, dependency leak check, biome/typecheck, package-file checks, and Chromium helper script tests are included in that gate. `rtk proxy env CARGO='rtk proxy cargo' just coverage` passed strict `--fail-under-lines 100 --fail-uncovered-lines 0` with `TOTAL ... Lines 8656, Missed Lines 0, Cover 100.00%`. `rtk cargo test -p katana-render-runtime --test html_browser_engine --locked` passed with 24 tests and no `KRR_CHROME_BIN`.
- [x] 4.4 `just VERSION=0.4.0 release-verify`、package 内容、publish dry-run、OpenSpec strict validation を通す。user approval 前に commit、push、PR、release を行わない。
  - Current verification: `rtk proxy env CARGO='rtk proxy cargo' just VERSION=0.4.0 release-verify` passed; Chromium archive install/check gate passed, tag safety and unpublished crates.io target checks passed, package checks passed, crate-size guard passed, and `cargo publish --dry-run` only was executed. `rtk scripts/openspec validate v0-4-0-html-dom-runtime --strict` passed. No commit, push, PR, publish, or release has been performed in the current prohibited-operation window.
