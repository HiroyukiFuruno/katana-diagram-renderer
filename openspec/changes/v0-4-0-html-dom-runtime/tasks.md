...
- [x] 4.3 KRR public API / dependency graph / rustfmt / clippy / workspace tests / `just coverage` を実行し、line coverage `100% / 0 uncovered` を通す。
  - Current verification: `rtk cargo test -p katana-render-runtime --test html_browser_engine --locked -- --test-threads=1` passed with 24 tests and no `KRR_CHROME_BIN`. `rtk proxy env CARGO='rtk proxy cargo' TEST_THREADS=1 just check` passed; AST lint, runtime asset checks, Chromium manifest/install helper checks, dependency leak check, biome/typecheck, package-file checks, and Chromium helper script tests are included in that gate. `rtk proxy env CARGO='rtk proxy cargo' TEST_THREADS=1 just coverage` passed strict `--fail-under-lines 100 --fail-uncovered-lines 0` with `TOTAL ... Lines 8719, Missed Lines 0, Cover 100.00%`. Windows CI surfaced unix-only browser test imports and helper fixtures that are now cfg-gated; local rechecks passed for `renderer_transfer`, `html_browser::process`, `just check`, and `just coverage`.
- [x] 4.4 `just VERSION=0.4.0 release-verify`、package 内容、publish dry-run、OpenSpec strict validation を通す。user approval 前に commit、push、PR、release を行わない。
  - Current verification: `rtk proxy env CARGO='rtk proxy cargo' TEST_THREADS=1 just VERSION=0.4.0 release-verify` passed; Chromium archive install/check gate passed, tag safety and unpublished crates.io target checks passed, package checks passed, crate-size guard passed, and `cargo publish --dry-run` only was executed. `rtk scripts/openspec validate v0-4-0-html-dom-runtime --strict` and `rtk proxy env VERSION=0.4.0 just release-openspec-archive` passed. KRR commit/push is proceeding after user approval; publish/release remains blocked until CI/release gate completion.

## 5. KDV And KatanA Handoff

- [ ] 5.1 KRR `0.4.0` が crates.io 公開済みであることを確認した後にだけ、KDV を crates.io `^0.4.0` に更新する。
  - Current verification: `rtk proxy env CARGO='rtk proxy cargo' just VERSION=0.4.0 release-verify` checked crates.io and confirmed the KRR target versions are unpublished. KDV work remains blocked until KRR `0.4.0` is actually published; KDV/KatanA local path dependencies remain prohibited, and no publish/release/push/commit was performed.
