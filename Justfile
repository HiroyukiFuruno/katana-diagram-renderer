...
pr-ready-check pr:
    ...
    pr_metadata="$(gh pr view "$pr" --json baseRefOid,headRefOid,headRefName,isDraft)"
    ...
    if ! IFS="$(printf '\011')" read -r base_sha head_sha branch parsed_repository trusted_default_sha readiness_mode extra < <(python3 -c 'import json, re, sys; pull = json.loads(sys.argv[1]); repository = sys.argv[2]; graph = json.loads(sys.argv[3]); data = graph.get("data") if isinstance(graph, dict) else None; node = data.get("repository") if isinstance(data, dict) else None; default = node.get("defaultBranchRef") if isinstance(node, dict) else None; target = default.get("target") if isinstance(default, dict) else None; fields = (pull.get("baseRefOid"), pull.get("headRefOid"), pull.get("headRefName"), repository, target.get("oid") if isinstance(target, dict) else None, pull.get("isDraft")); valid = isinstance(fields[0], str) and re.fullmatch(r"[0-9a-fA-F]{40}", fields[0]) and isinstance(fields[1], str) and re.fullmatch(r"[0-9a-fA-F]{40}", fields[1]) and isinstance(fields[2], str) and not any(ord(character) < 32 or ord(character) == 127 for character in fields[2]) and re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", fields[3]) and isinstance(default, dict) and isinstance(default.get("name"), str) and re.fullmatch(r"[A-Za-z0-9._/-]+", default["name"]) and isinstance(target, dict) and target.get("__typename") == "Commit" and isinstance(fields[4], str) and re.fullmatch(r"[0-9a-fA-F]{40}", fields[4]) and isinstance(fields[5], bool); valid or sys.exit("gh returned incomplete or unsafe PR/default metadata"); print(*fields[:5], "require-draft" if fields[5] else "allow-ready", sep=chr(9))' "$pr_metadata" "$repository" "$default_metadata"); then
      echo "gh returned incomplete PR metadata" >&2
      exit 2
    fi
    if [[ -n "${extra:-}" || -z "$base_sha" || -z "$head_sha" || -z "$branch" || -z "$parsed_repository" || "$parsed_repository" != "$repository" || -z "$trusted_default_sha" || ( "$readiness_mode" != "require-draft" && "$readiness_mode" != "allow-ready" ) ]]; then
      echo "gh returned incomplete PR metadata" >&2
      exit 2
    fi
