...
pr-ready-check pr:
    ...
    if ! IFS="$(printf '\011')" read -r base_sha head_sha branch repository readiness_mode extra < <(python3 -c 'import json, re, sys; pull = json.loads(sys.argv[1]); repo = json.loads(sys.argv[2]); fields = (pull.get("baseRefOid"), pull.get("headRefOid"), pull.get("headRefName"), repo.get("nameWithOwner"), pull.get("isDraft")); valid = isinstance(fields[0], str) and re.fullmatch(r"[0-9a-fA-F]{40}", fields[0]) and isinstance(fields[1], str) and re.fullmatch(r"[0-9a-fA-F]{40}", fields[1]) and isinstance(fields[2], str) and not any(ord(character) < 32 or ord(character) == 127 for character in fields[2]) and isinstance(fields[3], str) and re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", fields[3]) and isinstance(fields[4], bool); valid or sys.exit("gh returned incomplete or unsafe PR metadata"); print(*fields[:4], "require-draft" if fields[4] else "allow-ready", sep=chr(9))' "$pr_metadata" "$repository_metadata"); then
      echo "gh returned incomplete PR metadata" >&2
      exit 2
    fi
    if [[ -n "${extra:-}" || -z "$base_sha" || -z "$head_sha" || -z "$branch" || -z "$repository" || ( "$readiness_mode" != "require-draft" && "$readiness_mode" != "allow-ready" ) ]]; then
      echo "gh returned incomplete PR metadata" >&2
      exit 2
    fi
