...
pub(super) fn click_event<'scope>(
    scope: &mut HtmlTryCatchScope<'scope, '_, '_, '_>,
    target: v8::Local<'scope, v8::Object>,
) -> Result<v8::Local<'scope, v8::Object>, HtmlRuntimeError> {
    let value = evaluate_value(scope, "krr-html-dom-click-event", CLICK_EVENT_SOURCE)?;
    let event = v8::Local::<v8::Object>::try_from(value).map_err(click_event_error)?;
    let key = v8::String::new(scope, "target").ok_or(HtmlRuntimeError::DomBridge(
        "click event target key allocation failed".to_string(),
    ))?;
    event
        .set(scope, key.into(), target.into())
        .ok_or(HtmlRuntimeError::DomBridge(
            "click event target assignment failed".to_string(),
        ))?;
    Ok(event)
}

pub(super) fn run_inline_click_handler(
    ...
    let value = evaluate_value(scope, "krr-html-inline-onclick", &source)?;
...
fn click_event_error<T>(_error: T) -> HtmlRuntimeError {
    HtmlRuntimeError::DomBridge("click event allocation failed".to_string())
}
...
mod tests {
    ...
    fn interaction_error_helpers_preserve_contract_messages() {
        ...
        assert!(matches!(
            click_event_error(()),
            HtmlRuntimeError::DomBridge(message) if message == "click event allocation failed"
        ));
        assert!(matches!(
