...
通常Issueへのcomment created/edited/deletedも同じ再評価起点である。created/editedはcurrent Issueとcomment ID・created/updated timestampをAPIで一致確認し、deletedは取得不能なcommentを信頼せずcurrent Issueの`updated_at`一致だけを用いる。いずれもclosing keywordが正確に一致する同一repositoryのopen non-Draft PRだけを全page列挙し、欠損・重複・型不正はfail-closedにする。scheduleはnative conversation resolutionを置き換えず、イベント欠落の収束補助でありmerge根拠にはしない。

Issue eventの対象は、列挙したPRのうち非Draftで、本文にGitHub closing keyword（`close`/`closes`/`closed`、`fix`/`fixes`/`fixed`、`resolve`/`resolves`/`resolved`）と、変更Issueの`#N`または同一repositoryの完全Issue URLを組み合わせたclosing referenceを持つものだけである。通常の`verify_pr_ready.py --allow-ready`契約は、PR rangeのcommitが参照する同一repositoryのIssue集合と、PR本文のclosing referenceから得られる同一repositoryのIssue集合を完全一致させる（不足も余分も拒否）ため、Issue eventの再検証対象とcommit契約の対象が一致する。通常は最大256件をmatrixで再評価するが、その前にtrusted default-branch preflight jobが各current headへ専用Appのpending contextを最大3回で投稿して旧successを先行失効させる。preflightとpublisherは同じPR番号のconcurrency groupを共有し、新generationのpreflightが旧publisherをcancelしてpendingを確定するまでreplacement publisherを開始しない。257件以上ではoverflow jobが対象を再ページング・再取得し、各current headへfailure contextを最大3回でbest-effort全件投稿して旧successを無効化する。resolver後にclosed/Draft化した対象は記録してskipし、残存open non-Draft対象の投稿を継続する。投稿失敗・重複・型不正・foreign/fork・API異常はfail-closedでjobを失敗させ、残りの投稿は継続する。token取得・一覧API・最終status POSTの完全障害、または極端な件数でのActions timeoutはstatus APIにCASがないため既存statusを原子的に取り消せない構造上の残余であり、ここでrequired CI contextが停止を保証するとは主張しない。overflow statusのtarget URLにはrun固定generationを記録する。publisherはpending statusのPOST応答から得た専用App status IDをfence基準にし、秒精度の時刻比較は使わない。fenceはIssue markerだけに限定せず、同一専用App・同一contextの自分より新しいstatus IDを全event種別で検出して旧workerのfinal POSTを停止する。対象URLのgeneration情報が重複・不正ならfail-closedにする。成功直前には現在のPR rangeとIssue contractに加え、`verify_pr_ready.py --allow-ready`とevent Issueの`updated_at`を再確認する。non-PR issue commentもcurrent Issue/commentを検証したうえで、closing referenceを持つ対象PRを再評価する。
